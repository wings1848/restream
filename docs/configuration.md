...
| `source.config.fail_cooldown` | `60` | Seconds to wait after a failed resolve before running yt-dlp again. Prevents the retry loop from hammering the extractor every few seconds, which gets the exit IP temp-banned by YouTube ("Sign in to confirm you're not a bot") for hours. Invalid values (unparseable, ≤0, `NaN`, `Inf`) fall back to the default `60`; values above `86400` (24h) are clamped. During an active cooldown the retry wait is driven by this value and can exceed `retry.max_interval` (the source's cooldown floor wins over the backoff cap). Per-pipeline: several pipelines from the same IP each run yt-dlp independently. |
