...
| `source.config.fail_cooldown` | `60` | Seconds to wait after a failed resolve before running yt-dlp again. Prevents the retry loop from hammering the extractor every few seconds, which gets the exit IP temp-banned by YouTube ("Sign in to confirm you're not a bot") for hours. Per-pipeline: several pipelines from the same IP each run yt-dlp independently. |
