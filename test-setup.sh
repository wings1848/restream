...
# 步骤：pot-provider sidecar → (可选) GHCR 镜像 → yt-dlp 直播状态验证 → 启动 restream
set -u
...
echo
echo "== [2/4] GHCR CI 镜像检查（可选，默认跳过） =="
# ghcr.io/wings1848/restream:v0.3.0 已过时；本地通常直接使用当前构建的
# restream:clean 镜像。如需检查远程镜像，先登录再指定实际存在的 tag：
#   echo "$(gh auth token)" | docker login ghcr.io -u wings1848 --password-stdin >/dev/null 2>&1
#   docker pull ghcr.io/wings1848/restream:<tag>
echo "(跳过 GHCR 拉取，直接使用本地 restream:clean)"

echo
echo "== [3/4] yt-dlp 直播状态验证（经 Clash 代理 7897 + PO Token） =="
docker run --rm --network host restream:clean \
...
docker run -d --name restream-test --network host \
  ...
  restream:clean --config /etc/restream/config.yaml
